...
class ProtossAgent(base_agent.BaseAgent):
    ...
    def step(self, obs, locationIsUp):
        cameraPosX = 1
        cameraPosY = 1
        ...
        print("Len gateway: " + str(len(gateways)))
        print("asdfadfsdsfa gateway: " + str(gateways))
        ...
        if len(pylons) == 1:
            if locationIsUp is True:
                return actions.FUNCTIONS.move_camera((45, 17))
            else:
                return actions.FUNCTIONS.move_camera((10, 52))
            #10,52 is dowrn

        if len(pylons) < 3:
            print("Len pylon: " + str(len(pylons)))
            if self.unit_type_is_selected(obs, units.Protoss.Probe):
            ...
            #
            #     return actions.FUNCTIONS.move_camera((pylon.x, pylon.y), (x, y))
        # if self.can_do(obs, actions.FUNCTIONS.Build_Pylon_screen.id):
    ...
    try:
        while True:
            with sc2_env.SC2Env(
                ...
                while True:
                    step_actions = [agent.step(timesteps[0], locationIsUp)]
                    if timesteps[0].last():
                        break
