...
class ProtossAgent(base_agent.BaseAgent):
    ...
    def step(self, obs):
        ...
        # step1_action = agent_helper.select_one_probe(self, obs)
        # step2_action = agent_helper.build_pylon(obs, 1, 1)
        # combined_action = [step1_action, step2_action]
        ...
        while True:
            with sc2_env.SC2Env(
                ...
                while True:
                    ...
                    if timesteps[0].last():
                        break
                    # for index in range(len(step_actions)):
                    #     action = step_actions[index]
                    timesteps = env.step(step_actions[0])
                    timesteps = env.step(step_actions[1])
